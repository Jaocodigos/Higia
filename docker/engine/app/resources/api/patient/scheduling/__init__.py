...
def mark_schedule(patient, doctor_id):
    ...
    doctor = Users.query.filter_by(id=doctor_id).with_entities(Users.full_name).first_or_404()
    patient_data = Users.query.filter_by(identifier=patient.identifier).with_entities(Users.id).first_or_404()
    data = request.json
